...
export const nodeRules: ReviewRule[] = [
...

  {
    ...
    check: (code) =>
      /(await\s+)?(fetch|axios|axios\.\w+)\(/.test(code) &&
      !/(try\s*{[\s\S]*?catch\s*\(|\.catch\()/.test(code),
    message: "Async network call detected without explicit error handling.",
    rationale:
      "Unhandled promise rejections can crash Node.js processes or cause silent failures.",
  },

  {
    ...
    rationale:
      "Swallowing errors makes debugging impossible and hides production failures.",
  },
...

  {
    ...
    rationale:
      "Missing await can cause unhandled rejections or incorrect execution order.",
  },
...

  {
    ...
    rationale:
      "Controllers should delegate business logic to services to preserve SRP.",
  },

  {
    ...
    check: (code) =>
      /app\.(get|post|put|delete)\([\s\S]*?\)\s*{[\s\S]*?}/.test(code) &&
      !/res\.status\(/.test(code),
    message: "Express route handler does not explicitly set HTTP status codes.",
    rationale:
      "Implicit 200 responses can hide failures and break API contracts.",
  },
...

  {
    ...
    rationale:
      "Depending on concrete implementations instead of abstractions reduces testability and flexibility.",
  },
...

  {
    ...
    rationale:
      "Unvalidated input is a common source of injection and logic vulnerabilities.",
  },
...

  {
    ...
    check: (code) =>
      /(fs\.readFileSync|fs\.writeFileSync|child_process\.execSync)/.test(code),
